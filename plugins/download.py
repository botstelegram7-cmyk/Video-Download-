...
from downloader.media import remux, video_thumb, pdf_thumb, build_caption, add_meta
...
# ── URL handler ──────────────────────────────────────────────────────────────
@app.on_message(filters.text & ~filters.command(["start","help","ping","status","plans",
    "mystats","history","queue","cancel","settings","feedback","audio","info",
    "givepremium","removepremium","ban","unban","broadcast","stats","users","banned","restart"])
    & ~filters.outgoing)
@guard
...
async def cmd_info(client: Client, msg: Message):
    ...
    try:
        ...
        await m.edit(f"❌ Could not fetch info: {e}")
...
async def txt_handler(client: Client, msg: Message):
    ...
    try:
        ...
        cleanup(path)
    if not urls:
        await m.edit("❌ No URLs found in the file.")
        return
    await m.edit(f"»»──── 📋 Queuing {len(urls)} URL(s)… ────««")
    for url in urls:
        await _enqueue_download(client, msg, url, audio_only=False, silent=True)
    await m.edit(f"»»──── ✅ Queued {len(urls)} URLs ────««\n\n▸ Processing started!\n\n⋆｡° ✮ @Universal_DownloadBot ✮ °｡⋆")
...
async def _processor(item: QueueItem):
    ...
    username  = (user.get("username") or "user") if user else "user"
    ...
    try:
        if ext in VIDEO_EXTS:
            ...
            # Optionally remux for streaming
            remuxed = path.replace(f".{ext}", "_remux.mp4")
            ...
            await client.send_document(
                msg.chat.id, path,
                caption=caption,
                file_name=os.path.basename(path)   # preserve original filename
            )
