...
async def ytdlp_download(url, user_id, progress_cb=None, audio_only=False):
    import yt_dlp
    ...
    return {
        ...
        "ext":       os.path.splitext(found)[1].lstrip(".") if found else ext,
        "url_type":  url_type,
...
async def direct_download(url, user_id, progress_cb=None):
    out_dir  = temp_dir_for_user(user_id)
...
async def m3u8_download(url, user_id, progress_cb=None):
    out_dir  = temp_dir_for_user(user_id)
...
async def terabox_download(url, user_id, progress_cb=None):
    try:
...
async def _terabox_api(url, user_id, progress_cb=None):
    api_url = (
        "https://teraboxapp.com/api/shorturlinfo?app_id=250528&web=1"
        "&channel=dubox&clienttype=0&jsToken=&dp-logid=&dp-callid=&tsNow=1683016476498"
        "&shorturl=" + url
    )
...
async def download_url(url, user_id, progress_cb=None, audio_only=False):
    url_type = detect_url_type(url)
    logger.info("Download [%s]: %s", url_type, url[:80])
